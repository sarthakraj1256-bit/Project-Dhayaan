...
const GoalCard = ({
  ...
  const formatMinutes = (mins: number) => {
    if (mins >= 60) {
      ...
      return remaining > 0
        ? `${hours}${getLocalizedUnit('h', language)} ${remaining}${getLocalizedUnit('m', language)}`
        : `${hours}${getLocalizedUnit('h', language)}`;
    }
    return `${mins}${getLocalizedUnit('m', language)}`;
  };

  const isWeekly = type === 'weekly';
  const Icon = isWeekly ? Target : Calendar;
  const color = isWeekly ? 'cyan' : 'violet';
  const label = isWeekly ? t('sonic.weeklyGoal') : t('sonic.monthlyGoal');
        ...
        {isEditing ? (
          ...
          >
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {isSaving ? t('sonic.saving') : t('sonic.setGoal')}
              </button>
              <button
          ...
          >
            ...
            {/* Footer */}
            <div className="flex items-center justify-between text-xs text-muted-foreground">
              <span>{progress.percentage}% {t('sonic.complete')}</span>
              <span className="flex items-center gap-1">
                <Flame className="w-3 h-3" />
                {progress.daysRemaining} {t('sonic.daysLeft')}
              </span>
            </div>
          </motion.div>
        ) : (
          ...
          >
            <span>{isWeekly ? t('sonic.setWeeklyGoal') : t('sonic.setMonthlyGoal')}</span>
