...
export default function FinalStressModal({ isOpen, onClose, onSubmit }: FinalStressModalProps) {
  ...
  const calculateReduction = (): { percentage: number; improved: boolean } | null => {
    ...
    return {
      percentage: Math.abs(Math.round(reduction)),
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="glass-card border-primary/20 max-w-md mx-auto bg-background/95 backdrop-blur-xl">
        <DialogHeader className="text-center space-y-4 pt-4">
          ...
          <DialogDescription className="font-body text-muted-foreground text-base">
            After exploring the sacred architecture, rate your current stress level.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="glass-card p-4 bg-primary/5 border-primary/10 text-center space-y-2">
              <div className="flex items-center justify-center gap-4 text-sm">
                <div className="text-center">
                  <p className="text-muted-foreground text-xs tracking-wider">BEFORE</p>
                  <p className="font-display text-xl text-foreground">{initialStress}</p>
                </div>
                <span className="text-primary text-xl">→</span>
                <div className="text-center">
                  <p className="text-muted-foreground text-xs tracking-wider">NOW</p>
                  <p className="font-display text-xl text-gold-gradient">{stressLevel}</p>
                </div>
              </div>
              
              <p className={`font-display text-lg tracking-wider ${reductionData.improved ? 'text-green-400' : 'text-amber-400'}`}>
                {reductionData.improved ? '↓' : '↑'} {reductionData.percentage}% {reductionData.improved ? 'Reduction' : 'Increase'}
              </p>
              
              {reductionData.improved && reductionData.percentage > 0 && (
                <p className="text-xs text-muted-foreground font-body">
                  The sacred geometry is working! ✨
                </p>
              )}
            </div>
          )}
        ...
        <div className="flex flex-col gap-3 pb-2">
          <Button
            ...
            className="w-full font-display tracking-wider uppercase text-sm bg-primary/90 hover:bg-primary text-primary-foreground transition-all duration-300 hover:scale-[1.02] hover:shadow-lg hover:shadow-primary/20"
          >
            Submit Experience
          </Button>
        </div>
