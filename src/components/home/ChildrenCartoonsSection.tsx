    ...
    <>
      <section className="px-6 py-5">
        ...
        <div className="flex gap-3 overflow-x-auto pb-2 -mx-1 px-1 scrollbar-hide">
          ...
          <PlaylistCard title="Chhota Bheem aur Krishna ki Jodi" episodes={chhotaBheemKrishnaCartoons} onOpen={() => setShowBheemKrishna(true)} />
          <PlaylistCard title="Selfie with Bajrangi" episodes={selfieWithBajrangiCartoons} onOpen={() => setShowSelfieBajrangi(true)} />
          <PlaylistCard title="Jay Jagannath" episodes={jayJagannathCartoons} onOpen={() => setShowJayJagannath(true)} />
      ...
      {showRollNo21 && <EpisodeListModal title="Roll No 21" episodes={rollNo21Cartoons} onClose={() => setShowRollNo21(false)} onSelectVideo={handleSelect} />}
      {showBheemKrishna && <EpisodeListModal title="Chhota Bheem aur Krishna ki Jodi" episodes={chhotaBheemKrishnaCartoons} onClose={() => setShowBheemKrishna(false)} onSelectVideo={handleSelect} />}
      {showSelfieBajrangi && <EpisodeListModal title="Selfie with Bajrangi" episodes={selfieWithBajrangiCartoons} onClose={() => setShowSelfieBajrangi(false)} onSelectVideo={handleSelect} />}
      {showJayJagannath && <EpisodeListModal title="Jay Jagannath" episodes={jayJagannathCartoons} onClose={() => setShowJayJagannath(false)} onSelectVideo={handleSelect} />}
    </>
  );
}

const cardClass = "block w-full text-left rounded-2xl overflow-hidden light-card transition-all duration-200 active:scale-[0.98] focus-visible:ring-2 focus-visible:ring-primary outline-none";
...
function RollNo21PlaylistCard({ onOpen }: { onOpen: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="shrink-0 w-[172px]">
      <button type="button" aria-label="Open Roll No 21 playlist" onClick={onOpen} className={cardClass}>
        <div className="aspect-[16/9] relative overflow-hidden bg-muted">
          <img src={thumb} alt="Roll No 21" className="w-full h-full object-cover" loading="lazy" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent" />
          <div className="absolute top-1.5 left-1.5">
            <Badge className="bg-indigo-600 text-white border-0 text-[9px] px-1.5 py-0.5 leading-none">📚 {rollNo21Cartoons.length} Episodes</Badge>
          </div>
          <div className="absolute bottom-1.5 right-1.5 flex items-center gap-0.5 bg-black/50 backdrop-blur-sm px-1.5 py-0.5 rounded text-[10px] text-white/80">
            <ListVideo className="w-2.5 h-2.5" /> Playlist
          </div>
        </div>
        <div className="p-2.5">
          <p className="text-xs font-medium text-foreground/85 line-clamp-2 leading-snug">Roll No 21</p>
        </div>
      </button>
    </motion.div>
  );
...
function PlaylistCard({ title, episodes, onOpen }: { title: string; episodes: CartoonVideo[]; onOpen: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="shrink-0 w-[172px]">
      <button type="button" aria-label={`Open ${title} playlist`} onClick={onOpen} className={cardClass}>
        <div className="aspect-[16/9] relative overflow-hidden bg-muted">
          <img src={thumb} alt={title} className="w-full h-full object-cover" loading="lazy" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent" />
          <div className="absolute top-1.5 left-1.5">
            <Badge className="bg-indigo-600 text-white border-0 text-[9px] px-1.5 py-0.5 leading-none">📚 {episodes.length} Episodes</Badge>
          </div>
          <div className="absolute bottom-1.5 right-1.5 flex items-center gap-0.5 bg-black/50 backdrop-blur-sm px-1.5 py-0.5 rounded text-[10px] text-white/80">
            <ListVideo className="w-2.5 h-2.5" /> Playlist
          </div>
        ...
        <div className="p-2.5">
          <p className="text-xs font-medium text-foreground/85 line-clamp-2 leading-snug">{title}</p>
        </div>
      </button>
    </motion.div>
  );
          ...
          <div className="flex flex-col gap-3">
            {episodes.map((item, i) => (
              <button
                ...
                className="w-full text-left rounded-2xl overflow-hidden light-card transition-all duration-200 active:scale-[0.98] focus-visible:ring-2 focus-visible:ring-primary outline-none"
              >
                <div className="flex gap-3 p-2">
                  ...
                  <div className="flex flex-col justify-center min-w-0 py-1">
                    <p className="text-sm font-medium text-foreground/85 leading-snug">Episode {i + 1}</p>
...
function CartoonCard({ item, index, onSelect }: { item: CartoonVideo; index: number; onSelect: (v: CartoonVideo) => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.04 }} className="shrink-0 w-[172px]">
      <button type="button" aria-label={isPlaylist ? `Open playlist: ${item.title}` : `Play ${item.title}`} onClick={() => onSelect(item)} className={cardClass}>
        <div className="aspect-[16/9] relative overflow-hidden bg-muted">
