...
interface StressCheckInModalProps {
...
export default function StressCheckInModal({ onComplete }: StressCheckInModalProps) {
  ...
  useEffect(() => {
    // Check if user has recently completed the check-in
    ...
    if (storedTimestamp && storedStress) {
      ...
      if (elapsed < SESSION_DURATION) {
        // Session still valid, don't show modal
        return;
      }
    }
      ...
      <DialogContent className="glass-card border-primary/20 max-w-md mx-auto bg-background/95 backdrop-blur-xl">
        ...
        <DialogHeader className="text-center space-y-4 pt-4">
          ...
          <DialogDescription className="font-body text-muted-foreground text-base">
            Before you begin your journey into sacred geometry, share your current state of mind.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="py-8 space-y-8">
          ...
          <div className="glass-card p-4 bg-primary/5 border-primary/10">
            <p className="text-xs text-muted-foreground text-center font-body leading-relaxed">
              <span className="text-primary">॥</span> Studies show that exposure to sacred geometry 
              and temple architecture can reduce cortisol levels by up to 25% <span className="text-primary">॥</span>
            </p>
          </div>
        </div>
...
export function getInitialStress(): number | null {
  const stored = localStorage.getItem(STRESS_STORAGE_KEY);
