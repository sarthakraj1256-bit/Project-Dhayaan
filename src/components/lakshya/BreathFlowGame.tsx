...
type GameState = 'intro' | 'playing' | 'paused' | 'complete';
...
const BreathFlowGame = ({ onClose, onKarmaEarned }: BreathFlowGameProps) => {
  ...
  useEffect(() => {
    ...
    const gameLoop = () => {
      ...
      const cyclePosition = (now % CYCLE_TIME);
      
      if (cyclePosition < INHALE_TIME) {
        setBreathPhase('inhale');
        setBreathProgress(cyclePosition / INHALE_TIME);
      } else if (cyclePosition < INHALE_TIME + HOLD_TIME) {
        setBreathPhase('hold');
        setBreathProgress((cyclePosition - INHALE_TIME) / HOLD_TIME);
      } else {
        setBreathPhase('exhale');
        setBreathProgress((cyclePosition - INHALE_TIME - HOLD_TIME) / EXHALE_TIME);
      }

      // Update distance (energy travels forward during inhale)
      if (breathPhase === 'inhale') {
        setDistance(d => d + deltaTime * 0.01);
      }

      // Update consistency based on breathing rhythm
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Draw energy orb
      const orbSize = breathPhase === 'inhale' 
        ? 20 + breathProgress * 15 
        : breathPhase === 'exhale' 
          ? 35 - breathProgress * 15 
          : 35;
  ...
  const completeGame = useCallback(async () => {
    ...
    const totalKarma = Math.max(10, timeBonus + distanceBonus + consistencyBonus);
  ...
  }, [elapsedTime, distance, consistency, userId, addKarma, onKarmaEarned]);
        ...
        <AnimatePresence>
          {gameState === 'intro' && (
            ...
            >
              ...
              <h3 className="font-display text-2xl text-foreground mb-4">Breath Flow Journey</h3>
              <p className="text-muted-foreground text-center max-w-md mb-8">
                Control the glowing energy with your breath. Inhale slowly to move forward, 
                hold to gather power, exhale to release and stabilize.
              </p>
              
              <div className="grid grid-cols-3 gap-4 mb-8 text-center">
                <div className="p-3 rounded-lg bg-cyan-500/10 border border-cyan-500/20">
                  <p className="text-sm text-cyan-400">Inhale</p>
                  <p className="text-xs text-muted-foreground">4 seconds</p>
                </div>
                <div className="p-3 rounded-lg bg-violet-500/10 border border-violet-500/20">
                  <p className="text-sm text-violet-400">Hold</p>
                  <p className="text-xs text-muted-foreground">2 seconds</p>
                </div>
                <div className="p-3 rounded-lg bg-indigo-500/10 border border-indigo-500/20">
                  <p className="text-sm text-indigo-400">Exhale</p>
                  <p className="text-xs text-muted-foreground">6 seconds</p>
                </div>
              </div>

              <button
                onClick={startGame}
                className="px-8 py-3 rounded-full bg-gradient-to-r from-cyan-500 to-indigo-500 text-white font-medium hover:opacity-90 transition-opacity flex items-center gap-2"
              >
        ...
        <AnimatePresence>
          {gameState === 'complete' && (
            ...
            >
              ...
              >
                <div className="text-5xl font-display text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-orange-400 mb-2">
                  +{karmaEarned}
                </div>
              ...
              <div className="flex gap-4">
                ...
                <button
                  onClick={onClose}
                  className="px-6 py-2 rounded-full bg-primary/20 border border-primary/50 text-primary hover:bg-primary/30 transition-colors"
                >
                  Done
                </button>
              </div>
          ...
          <>
            ...
            <div className="absolute top-4 right-4 flex items-center gap-2">
              <button
                ...
                className="p-2 rounded-full bg-void/80 backdrop-blur-sm hover:bg-void transition-colors"
              >
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 w-[calc(100%-2rem)] flex flex-col items-center gap-3">
              <div className="px-6 py-3 rounded-full bg-void/80 backdrop-blur-sm flex items-center gap-4">
                <motion.div
                  ...
                  transition={{ duration: breathPhase === 'inhale' ? 4 : breathPhase === 'exhale' ? 6 : 2, repeat: Infinity }}
                  className="w-8 h-8 rounded-full bg-gradient-to-r from-cyan-400 to-indigo-400"
                />
                <div className="text-center">
                  <p className={`text-lg font-medium capitalize ${
                    breathPhase === 'inhale' ? 'text-cyan-400' : 
                    breathPhase === 'hold' ? 'text-violet-400' : 'text-indigo-400'
                  }`}>
                    {breathPhase}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {breathPhase === 'inhale' ? 'Breathe in slowly...' : 
                     breathPhase === 'hold' ? 'Hold your breath...' : 'Release slowly...'}
                  </p>
                </div>
              </div>
              <button
            ...
            {gameState === 'paused' && (
              <div className="absolute inset-0 flex items-center justify-center bg-void/50 backdrop-blur-sm">
                <div className="text-center">
                  ...
                  <button
                    onClick={resumeGame}
                    className="px-6 py-2 rounded-full bg-primary/20 border border-primary/50 text-primary"
                  >
                    Resume
                  </button>
                </div>
