...
import { ArrowLeft, Play, Clock, User, Sparkles, Music, Zap, BookOpen, Video, Filter } from 'lucide-react';
...
function ContentCard({ item, index }: { item: SpiritualContent; index: number }) {
  const { t } = useLanguage();
  ...
  return (
    ...
    >
      <a
        ...
        className="block rounded-2xl overflow-hidden bg-[#EDE8E0] dark:bg-[#1C1917] shadow-sm hover:shadow-md transition-all duration-200 active:scale-[0.98] no-underline group"
      >
        ...
        <div className="p-2.5">
          ...
          <p className="text-[10px] text-[#9C8C7C] mt-0.5">Opens in YouTube →</p>
        </div>
      </a>
    </motion.div>
  );
